...
const vueApp = {
    ...
    data() {
        const [vorgangsId, aufgabenId] = location.search.substring(1).split('&')
        return {
            aufgabe: undefined,
    ...
    async created() {
        ...
        const matchingStatuses = await Aufgabenstatus.query(`SELECT * FROM Aufgabenstatuses WHERE VorgangsId='${this.vorgangsId}' AND AufgabenId='${this.aufgabenId}'`)
        if (matchingStatuses.length > 0) {
            this.aufgabenstatus = matchingStatuses[0]
        } else {
            this.aufgabenstatus = new Aufgabenstatus({ VorgangsId: this.vorgangsId, AufgabenId: this.aufgabenId, Status: 'offen' })
